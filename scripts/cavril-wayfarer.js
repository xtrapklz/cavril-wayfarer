...
const Store = (() => {
    ...
    function register() {
            ...
            if (FA && g.registerMenu) {
                const SoundsApp = class extends FA {
                    ...
                    async _updateObject(event, formData) {
                        ...
                        try { await game.settings.set(MOD, "travelSfx", !!formData.travelSfx); } catch (e) { warn("travelSfx save failed", e); }
                        try { await game.settings.set(MOD, "travelSfxPath", String(formData.travelSfxPath || "").trim()); } catch (e) { warn("travelSfxPath save failed", e); }
        ...
        g.register(MOD, "travelSfx", { name: "Travel movement sounds", hint: "Play a one-shot movement sound (via Maestro) as the party crosses each hex — foot, cart, or boat matched to the terrain + your boat/cart toggle. Point it at your sound FOLDER below.", scope: "world", config: false, type: Boolean, default: false });   // config:false → folded into the 🎵 Sound cues menu (all SFX in one place)
            ...
            <div class="cwf-body" ${collapsedRef ? 'style="display:none"' : ""}>
                ...
                <div class="cwf-section">
                    ...
                    ${isGM ? `<div class="cwf-chips">
                        <button class="cwf-chip chip-danger ${_dialsOpen ? "on" : ""}" data-action="toggle-dials" title="Region dials — DANGER (how OFTEN combat fires · doubled by day, biome adds) · CHALLENGE (how HARD · skill DCs + foe budget) · WANTED (your Heat / notoriety · roads expose, deadly biomes hide). Click to tune."><i class="fa-solid fa-skull" style="color:#d8665a"></i> ${dangerNow}<span style="opacity:.4;margin:0 3px">·</span><i class="fa-solid fa-gauge-high" style="color:#4f9fe6"></i> ${cwfChallenge()}<span style="opacity:.4;margin:0 3px">·</span><i class="fa-solid fa-star" style="color:#f2c64b"></i> ${cwfWanted()}</button>
